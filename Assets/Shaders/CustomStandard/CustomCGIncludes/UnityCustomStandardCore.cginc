...
// Gone Mad Studios -- Custom Standard Core
...
struct VertexOutputForwardBase
{
	...
	float2 dissolveCoord				: TEXCOORD9;
#endif
...
VertexOutputForwardBase vertForwardBase (VertexInput v)
{
	...
	// Dissolve Coords
			...
			o.dissolveCoord = WorldCoordDissolve(posWorld,normalWorld);
		#endif
	#endif
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	float2 dissolveCoord = i.tex.xy;
	#if defined(_DISSOLVE) && (defined(_DISSOLVE_WORLD_SPACE) || defined(_DISSOLVE_LOCAL_SPACE))
		dissolveCoord = i.dissolveCoord;
	#endif

	//Dissolve Clip
	#if defined(_DISSOLVE)
		clip(ClampDissolve(dissolveCoord));
	#endif
    ...
    //Posiciones globales y normales para la posición global del dissolve
  	float3 posWorld;
  	#if UNITY_REQUIRE_FRAG_WORLDPOS
      #if UNITY_PACK_WORLDPOS_WITH_TANGENT
  			posWorld.x = i.tangentToWorldAndPackedData[0].w;
  			posWorld.y = i.tangentToWorldAndPackedData[1].w;
  			posWorld.z = i.tangentToWorldAndPackedData[2].w;
  		#else
  			posWorld = i.posWorld.xyz;
  		#endif
  	#endif
  	float4 normalWorld[3];
  	normalWorld[0] = i.tangentToWorldAndPackedData[0];
  	normalWorld[1] = i.tangentToWorldAndPackedData[1];
  	normalWorld[2] = i.tangentToWorldAndPackedData[2];

    //Emission
    half4 c = UNITY_BRDF_PBS (s.diffColor, s.specColor, s.oneMinusReflectivity, s.smoothness, s.normalWorld, -s.eyeVec, gi.light, gi.indirect);
    c.rgb += Emission(i.tex.xy, dissolveCoord);
...
)
{
    ...
    emissiveColor += Emission (i.tex.xy, dissolveCoord);
